...
#### Import train data:
train <- as.data.frame(read_excel("E2 - train data to score.xlsx",
                                  sheet = "Score_train"))
...
m0<- glm(death ~   age + gen_sc1 + ph_mean + hosp_sc1 + dc_af_sc_sum 
         , binomial, strain)
summary(m0)
...
m1<- glm(death ~  age + gen_sc1 + ph_mean + hosp_sc1 + dc_af_sc_sum + age:gen_sc1 +
...
# Best cutoff -  closest intersection
both.eq <- which.min(abs(sensitivity@y.values[[1]]-specificity@y.values[[1]]))
specificity@x.values[[1]][both.eq]

# Area under the curve:
(area <- performance(pred, "auc"))#0.811
